...
public class emp_service_emp implements employeeservice {
	...
	public void deleteusers(int id) {
		...
		employee e=this.repo.findById(id).orElseThrow(()->new ResourceNotFoundException("Application","id",id));
		this.repo.delete(e);
	...
	public List<employee_payload> getall() {
		// TODO Auto-generated method stub
		List<employee> li=(List<employee>) this.repo.findAll();
		List<employee_payload> emp=li.stream().map(employee->this.employee_dto(employee)).collect(Collectors.toList());
		return emp;
	}
